...
echo " Running Pipeline for Frame: $FRAME"
echo " Features Directory: $FEATURES_DIR"

# export_features.py 
echo "--> Exporting feature CSVs..."

python3 scripts/export_features.py --config $CONFIG --level gen --chunk 0
python3 scripts/export_features.py --config $CONFIG --level gen --component sm --chunk 0
python3 scripts/export_features.py --config $CONFIG --level reco --chunk 0
python3 scripts/export_features.py --config $CONFIG --level reco --component sm --chunk 0
  ...
  for level in gen reco; do
    for lepton in electron muon all; do
      ...
      if [ "$lepton" = "all" ]; then
        ...
        cpv_tag="${level}_${lepton}"
        sm_tag="sm_${level}_${lepton}"
      fi

      # CPV
      python3 scripts/build_angular_observable.py \
        --config $CONFIG \
        --features $FEATURES_DIR/features_${level}_${FRAME}_chunk0.csv \
      ...
      python3 scripts/build_angular_observable.py \
        --config $CONFIG \
        --features $FEATURES_DIR/features_sm_${level}_${FRAME}_chunk0.csv \
        --observable $observable \
        --split all \
...
for observable in O_W O_lD; do
  for level in gen reco; do
    for lepton in electron muon; do
      ...
      TEMPLATE="${OBS_DIR}/${observable}_all_${level}_${lepton}_bins.csv"
      SM_TEMPLATE="${OBS_DIR}/${observable}_all_sm_${level}_${lepton}_bins.csv"

      echo "Evaluating Fisher Info: ${observable} | ${level} | ${lepton}"
...
echo "All exports and templates complete."